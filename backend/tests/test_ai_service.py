...
class TestGetModuleTag:
    def test_course_tools(self):
        for tool in ("create_course", "list_courses", "get_course", "update_course", "delete_course"):
            assert AIService._get_module_tag(tool) == "course"

    def test_student_tools(self):
        for tool in ("create_student", "list_students", "get_student", "update_student", "delete_student"):
            assert AIService._get_module_tag(tool) == "student"

    def test_data_tools(self):
        for tool in ("get_dashboard_stats", "get_course_statistics", "get_student_statistics", "get_monthly_trends"):
            assert AIService._get_module_tag(tool) == "data"

    def test_lesson_plan_tools(self):
        for tool in ("create_lesson_plan", "list_lesson_plans", "get_lesson_plan", "update_lesson_plan", "publish_lesson_plan", "delete_lesson_plan"):
    ...
    def test_notification_tools(self):
        for tool in ("list_notifications", "get_notification", "mark_notification_read", "mark_all_notifications_read", "delete_notification"):
...
class TestBuildMessages:
    ...
    async def test_tool_calls_json_parsed(self):
        db = AsyncMock()
        tool_calls_json = json.dumps([{"id": "tc1", "function": {"name": "create_course", "arguments": "{}"}, "type": "function"}])
...
class TestSaveMessage:
    ...
    async def test_save_message_basic(self):
        ...
        with patch("app.services.ai.AIMessage") as MockAIMessage:
            ...
            result = await AIService._save_message(db, "conv-1", "user", "hello")
...
class TestGetOrCreateConversation:
    ...
    async def test_new_conversation_created(self):
        ...
        with patch.object(AIService, "_get_next_session_number", new_callable=AsyncMock, return_value=1) as mock_next:
...
class TestNonStreamChat:
    ...
    async def test_normal_response(self):
        ...
        with patch("app.services.ai.httpx.AsyncClient") as MockClient:
            ...
            with patch.object(AIService, "_save_message", new_callable=AsyncMock) as mock_save:
    ...
    async def test_tool_calls_execution(self):
        ...
        messages = [{"role": "system", "content": "prompt"}, {"role": "user", "content": "list courses"}]
        effective_config = {
            "api_key": "test-key",
            "api_base": "https://api.test.com/v1",
            "model": "test-model",
        ...
        with patch("app.services.ai.httpx.AsyncClient") as MockClient:
            ...
            with patch.object(AIService, "_save_message", new_callable=AsyncMock) as mock_save:
                with patch("app.services.ai.TOOL_EXECUTOR", {"list_courses": AsyncMock(return_value={"items": [], "total": 0})}) as mock_executors:
                    result = await AIService._non_stream_chat(db, "user-1", conversation, messages, effective_config)
...
class TestToolExecutor:
    def test_all_expected_tool_names_present(self):
        expected_tools = [
            ...
            "create_student", "list_students", "get_student", "update_student", "delete_student",
            "get_dashboard_stats", "get_course_statistics", "get_student_statistics", "get_monthly_trends",
            "create_lesson_plan", "list_lesson_plans", "get_lesson_plan", "update_lesson_plan",
            "publish_lesson_plan", "delete_lesson_plan",
            "list_resources", "get_resource", "update_resource", "delete_resource",
            "list_notifications", "get_notification", "mark_notification_read",
