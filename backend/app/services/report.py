...
class ReportService:
    ...
    @staticmethod
    async def get_course_statistics(db: AsyncSession, user_id: Optional[str] = None) -> Dict[str, Any]:
        course_filters = [Course.is_deleted == False]
        if user_id is not None:
            course_filters.append(Course.user_id == user_id)
    ...
    @staticmethod
    async def get_student_statistics(db: AsyncSession, user_id: Optional[str] = None) -> Dict[str, Any]:
        student_filters = [Student.is_deleted == False]
        if user_id is not None:
            student_filters.append(Student.user_id == user_id)
