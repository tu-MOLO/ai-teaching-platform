...
class TagService:
    ...
    async def count_tags(db: AsyncSession) -> int:
        """统计未删除标签总数"""
        try:
            query = select(func.count(Tag.id)).where(Tag.is_deleted == False)  # noqa: E712
            result = await db.execute(query)
    ...
    async def get_tag_by_id(db: AsyncSession, tag_id: str) -> Optional[Tag]:
        ...
        """
        try:
            query = select(Tag).where(Tag.id == tag_id, Tag.is_deleted == False)  # noqa: E712
            result = await db.execute(query)
    ...
    async def get_tag_by_name(db: AsyncSession, name: str) -> Optional[Tag]:
        ...
        """
        try:
            query = select(Tag).where(Tag.name == name, Tag.is_deleted == False)  # noqa: E712
            result = await db.execute(query)
            tag = result.scalar_one_or_none()
            return tag
        except Exception as e:
    ...
    async def get_tags_by_ids(db: AsyncSession, tag_ids: List[str]) -> List[Tag]:
        ...
        """
        try:
            query = select(Tag).where(Tag.id.in_(tag_ids), Tag.is_deleted == False)  # noqa: E712
            result = await db.execute(query)
            tags = result.scalars().all()
            return list(tags)
        except Exception as e:
