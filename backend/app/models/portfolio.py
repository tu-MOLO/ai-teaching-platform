...
class Portfolio(BaseModel):
    ...
    attachments: Mapped[str] = mapped_column(Text, nullable=True, comment="附件（JSON格式）")

    # 多维度评价字段（0-100分）
    cognitive_score: Mapped[int] = mapped_column(
        Integer, nullable=True, comment="认知维度评分（0-100）"
    )

    skill_score: Mapped[int] = mapped_column(
        Integer, nullable=True, comment="技能维度评分（0-100）"
    )

    creativity_score: Mapped[int] = mapped_column(
        Integer, nullable=True, comment="创意维度评分（0-100）"
    )

    cooperation_score: Mapped[int] = mapped_column(
        Integer, nullable=True, comment="合作维度评分（0-100）"
    )

    attention_score: Mapped[int] = mapped_column(
        Integer, nullable=True, comment="注意力维度评分（0-100）"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "cognitive_score IS NULL OR (cognitive_score >= 0 AND cognitive_score <= 100)",
            name="ck_cognitive_score_range",
        ),
        CheckConstraint(
            "skill_score IS NULL OR (skill_score >= 0 AND skill_score <= 100)",
            name="ck_skill_score_range",
        ),
        CheckConstraint(
            "creativity_score IS NULL OR (creativity_score >= 0 AND creativity_score <= 100)",
            name="ck_creativity_score_range",
        ),
        CheckConstraint(
            "cooperation_score IS NULL OR (cooperation_score >= 0 AND cooperation_score <= 100)",
            name="ck_cooperation_score_range",
        ),
        CheckConstraint(
            "attention_score IS NULL OR (attention_score >= 0 AND attention_score <= 100)",
            name="ck_attention_score_range",
        ),
    )
