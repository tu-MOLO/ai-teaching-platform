...
class QueryOptimizer:
    ...
    def selectinload_chain(*paths: str):
        ...
        option = selectinload(paths[0])  # type: ignore[arg-type]
        for path in paths[1:]:
            option = option.selectinload(path)  # type: ignore[arg-type]
...
def cached(ttl: int = 300, key_func: Optional[Callable] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        # 添加清除缓存的方法
        wrapper.cache_clear = lambda: get_cache().delete(  # type: ignore[attr-defined]
            ":".join([func.__name__])
        )
