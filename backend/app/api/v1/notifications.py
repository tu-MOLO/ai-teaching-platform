...
async def get_notifications(
    ...
    notifications = await NotificationService.get_list(
        db, user_id=current_user, skip=offset, limit=page_size, notification_type=type, read=read
    )

    total = await NotificationService.count(
        db, user_id=current_user, notification_type=type, read=read
    )
...
async def get_notification_stats(db: DBSession, current_user: CurrentUser) -> NotificationStats:
    ...
    return NotificationStats(total=total, unread=unread, read=read, by_type=by_type)


@router.get("/unread-count", response_model=dict, summary="获取未读通知数量")
async def get_unread_count(db: DBSession, current_user: CurrentUser) -> dict:
...
async def mark_all_as_read(db: DBSession, current_user: CurrentUser) -> MessageResponse:
    updated_count = await NotificationService.mark_all_as_read(db, current_user)
    return MessageResponse(message=f"成功标记 {updated_count} 条通知为已读", code="success")


@router.put("/read-batch", response_model=MessageResponse, summary="批量标记通知为已读")
async def mark_batch_as_read(
    read_request: NotificationReadRequest, db: DBSession, current_user: CurrentUser
) -> MessageResponse:
    if read_request.ids:
...
async def delete_all_read(db: DBSession, current_user: CurrentUser) -> MessageResponse:
    deleted_count = await NotificationService.delete_all_read(db, current_user)
    return MessageResponse(message=f"成功删除 {deleted_count} 条已读通知", code="success")
...
async def get_notification(
    ...
    notification = await NotificationService.get(db, notification_id, user_id=current_user)
...
async def mark_notification_as_read(
    ...
    notification = await NotificationService.mark_as_read(db, notification_id, user_id=current_user)
...
async def delete_notification(
    ...
    success = await NotificationService.delete(db, notification_id, current_user)
